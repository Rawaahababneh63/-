...
namespace Masterpiece.Controllers
{
    ...
    public class CartController : ControllerBase
    {
        ...
        public IActionResult getUserCartItems(int UserId)
        {
            ...
            var cartItems = _db.CartItems
                ...
                .Select(x => new cartItemResponseDTO
                {
                    ...
                    Quantity = x.Quantity ?? 0, // استخدام القيمة الافتراضية إذا كانت فارغة
                }).ToList();
        ...
        //            Quantity = newItem.Quantity,
        //        };
        //        var product = _db.Products.FirstOrDefault(p => p.ProductId == newItem.ProductId);
        //        if (product == null)
        //        {
        ...
        //        _db.CartItems.Add(data);
        //        _db.SaveChanges();
        //        // Step 3: Reduce the quantity in stock
        ...
        public IActionResult AddCartItem([FromBody] addCartItemRequestDTO newItem, int UserId)
        {
            if (newItem == null || newItem.Quantity <= 0)
            {
                return BadRequest(new { message = "مدخلات غير صالحة." });
            }
            ...
            if (user == null)
            {
                return NotFound(new { message = "لم يتم العثور على سلة لهذا المستخدم." });
            }
            ...
            if (product.StockQuantity <= 0 || product.StockQuantity < newItem.Quantity)
            {
                return BadRequest(new { message = "الكمية المطلوبة غير متوفرة." });
            }
            ...
            if (checkSelectedProduct == null)
            {
                if (product.StockQuantity < newItem.Quantity)
                {
                    return BadRequest(new { message = $"الكمية المتاحة في المخزون هي {product.StockQuantity} فقط." });
                }
            ...
            {
                if (product.StockQuantity < newItem.Quantity + checkSelectedProduct.Quantity)
                {
                    return BadRequest(new { message = $"لا يمكن إضافة أكثر من {product.StockQuantity + checkSelectedProduct.Quantity} بسبب نقص الكمية في المخزون." });
                }
                ...
                return Ok(new { message = "تم تحديث كمية المنتج في السلة بنجاح." });
            }
        }
