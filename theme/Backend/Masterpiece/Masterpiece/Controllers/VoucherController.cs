...
namespace Masterpiece.Controllers
{
    ...
    public class VoucherController : ControllerBase
    {
      ...
      ";
        ...
        public IActionResult ChangeStatus(int id, [FromBody] StatusChangeRequest request)
        {
            // استرجاع طلب البيع مع المنتج المرتبط
            var saleRequest = _db.SaleRequests
                .Include(sr => sr.User)  // تضمين المستخدم
                .Include(sr => sr.Product)  // تضمين المنتج المرتبط بالطلب
            ...
            // التحقق من وجود حالة الطلب
            if (string.IsNullOrWhiteSpace(request.Status))
            {
                return BadRequest(new { message = "حقل الحالة مطلوب." });
            }

            // التعامل مع الحالة "موافقة" (approve)
            if (request.Status.Equals("approve", StringComparison.OrdinalIgnoreCase))
            {
                saleRequest.Status = "Approved";

                if (saleRequest.Product != null)  // تحقق من وجود المنتج
                {
                    saleRequest.Product.IsActive = true;  // تغيير حالة المنتج في الجدول
                }
                else
                {
                    return BadRequest(new { message = "المنتج المرتبط بالطلب غير موجود." });
                }

                // إرسال البريد الإلكتروني عند الموافقة
                SendEmailNotification(saleRequest.User.Email, saleRequest.User.UserName, true, null);
            }
            // التعامل مع الحالة "رفض" (reject)
            else if (request.Status.Equals("reject", StringComparison.OrdinalIgnoreCase))
            {
                if (string.IsNullOrWhiteSpace(request.RejectionReason))
                {
                ...
                // إرسال البريد الإلكتروني عند الرفض
                SendEmailNotification(saleRequest.User.Email, saleRequest.User.UserName, false, request.RejectionReason);
            }
            else
            {
            ...
            _db.SaveChanges();
            return NoContent();  // إرجاع استجابة بدون محتوى (حالة نجاح)
        }
