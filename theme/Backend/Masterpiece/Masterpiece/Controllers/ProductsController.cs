...
namespace Masterpiece.Controllers
{
    ...
    public class ProductsController : ControllerBase
    {
        ...
        {
            // التأكد من أن المنتجات تنتمي إلى فئة فرعية معينة وأنها فعّالة (IsActive = true)
            var products = await _db.Products
                .Where(p => p.IsActive == true &&
                            _db.Subcategories.Any(sc => sc.SubcategoryId == p.SubcategoryId && sc.CategoryId == categoryId))
                .ToListAsync();

            if (products == null || !products.Any())
            {
                return NotFound($"No active products found for category ID {categoryId}");
            }
        ...
        public async Task<IActionResult> FilterByPrice(decimal minPrice, decimal maxPrice, int mainCategoryId)
        {
            ...
            var filteredProducts = await _db.Products
                ...
                .Include(p => p.Subcategory)
                .Where(p => p.IsActive == true 
                && p.Price >= minPrice && p.Price <= maxPrice && p.Subcategory.CategoryId == mainCategoryId)
                .ToListAsync();
        ...
        public async Task<IActionResult> FilterByBrand(string brand, int mainCategoryId)
        {
            ...
            var filteredProducts = await _db.Products
                ...
                .Include(p => p.Subcategory)
                .Where(p => p.IsActive == true  
                && p.Brand.ToLower() == brand.ToLower() && p.Subcategory.CategoryId == mainCategoryId)
                .ToListAsync();
        ...
        public async Task<IActionResult> GetProductById(int id)
        {
            ...
            var productDto = new ProductDetailsDTO
            {
                ...
                Size = product.Size,
                NumericSize = product.NumericSize,
        ...
        public async Task<IActionResult> AddProductOld([FromForm] ProductDTO productDto)
        {
            ...
            {
                return BadRequest("يرجى تحديد الفئة الفرعية.");
            }
            var subcategoryExists = await _db.Subcategories.AnyAsync(sc => sc.SubcategoryId == productDto.SubcategoryId);
            if (!subcategoryExists)
            {
            ...
            var product = new Product
            {
                ...
                NumericSize = productDto.NumericSize,   
            ...
            if (productDto.Image1 != null)
            {
                ...
                product.Image1 = productDto.Image1.FileName;
            }
            ...
            if (productDto.Image2 != null)
            {
                ...
                product.Image2 = productDto.Image2.FileName;
            }
            ...
            if (productDto.Image3 != null)
            {
                ...
                product.Image3 = productDto.Image3.FileName;
            }
        ...
        public IActionResult EditProduct(int id, [FromForm] ProductDTO products)
        {
            ...
            // تحديث الخصائص فقط إذا كانت القيم موجودة
            if (!string.IsNullOrEmpty(products.Name)) EditProduct.Name = products.Name;
            if (!string.IsNullOrEmpty(products.Description)) EditProduct.Description = products.Description;
            if (products.Price > 0) EditProduct.Price = products.Price;
            if (products.MaxPrice > 0) EditProduct.MaxPrice = products.MaxPrice;
            if (products.MinPrice > 0) EditProduct.MinPrice = products.MinPrice;
            if (products.Brand != null) EditProduct.Brand = products.Brand;
            if (products.PriceWithDiscount.HasValue) EditProduct.PriceWithDiscount = products.PriceWithDiscount.Value;
            if (products.SubcategoryId > 0) EditProduct.SubcategoryId = products.SubcategoryId;
            if (products.TypeProduct != null) EditProduct.TypeProduct = products.TypeProduct;
            if (products.Condition != null) EditProduct.Condition = products.Condition;
            if (products.StockQuantity > 0) EditProduct.StockQuantity = products.StockQuantity;

            EditProduct.IsActive = products.IsActive;
            EditProduct.IsDonation = products.IsDonation;

            if (products.ClothColorId > 0) EditProduct.ClothColorId = products.ClothColorId;

            var folder = Path.Combine(Directory.GetCurrentDirectory(), "Uploads");
            if (!Directory.Exists(folder)) Directory.CreateDirectory(folder);

            SaveImage(products.Image, folder, img => EditProduct.Image = img);
            SaveImage(products.Image1, folder, img => EditProduct.Image1 = img);
            SaveImage(products.Image2, folder, img => EditProduct.Image2 = img);
            SaveImage(products.Image3, folder, img => EditProduct.Image3 = img);
        ...
        private async Task SaveImage(IFormFile imageFile, string folder, Product product, Action<string> setImage)
        {
            if (imageFile != null)
            {
        ...
        private void SaveImage(IFormFile imageFile, string folder, Action<string> setImage)
        {
            if (imageFile != null)
            {
                var uniqueFileName = $"{Guid.NewGuid()}_{imageFile.FileName}";
                var imagePath = Path.Combine(folder, uniqueFileName);
        ...
        [HttpGet("GetTopFiveThob")]
        public IActionResult GetTopFiveThob()
        {
            var subcategory = _db.Subcategories
                .Include(s => s.Products)
                .FirstOrDefault(s => s.SubcategoryName == "أثواب");

            if (subcategory == null)
            {
                return NotFound("Subcategory 'أثواب' not found.");
            }
        ...
        //GetTopFiveDressWedding

        [HttpGet("GetTopFiveDress")]
        public IActionResult GetTopFiveDress()
        {
            var subcategory = _db.Subcategories
                .Include(s => s.Products)
                .FirstOrDefault(s => s.SubcategoryName == "فساتين  الأعراس ");

            if (subcategory == null)
            {
                return NotFound("Subcategory 'فساتين  الأعراس ' not found.");
            }
        ...
        [HttpGet("GetTopFiveDressSahra")]
        public IActionResult GetTopFiveDressSahra()
        {
            var subcategory = _db.Subcategories
                .Include(s => s.Products)
                .FirstOrDefault(s => s.SubcategoryName == "فساتين حفلات");

            if (subcategory == null)
            {
                return NotFound("Subcategory 'فساتين حفلات' not found.");
            }
      ...
      .ToList();
            ...
            public async Task<IActionResult> BuyProduct([FromBody] BuyProductRequest request)
            {
                ...
                if (product.StockQuantity < request.QuantityToBuy)
                {
                    return BadRequest("الكمية المطلوبة أكثر من المتاحة.");
                }

                // تقليل الكمية في قاعدة البيانات
                product.StockQuantity -= request.QuantityToBuy;

                _db.Products.Update(product);
                await _db.SaveChangesAsync();

                return Ok(new { message = "تم شراء المنتج بنجاح" });
            }
