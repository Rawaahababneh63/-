...
async function submitForm(event) {
    ...
    const mainCategoryId =  localStorage.getItem('selectedCategoryId'); // حدد قيمة صحيحة لـ mainCategoryId حسب حاجتك
    formData.append('mainCategoryId', mainCategoryId);

    const isDonation = localStorage.getItem("IsDonation") || 0; // استرجاع حالة التبرع من localStorage (أو تعيين القيمة الافتراضية 0)

    // إذا كان تبرع، تعيين السعر إلى 0
    if (isDonation == 1) {
        formData.set('RequestedPrice', 0); // تعيين السعر 0 إذا كان التبرع
    }
